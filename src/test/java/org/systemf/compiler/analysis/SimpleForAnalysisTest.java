...
public class SimpleForAnalysisTest {
	public static void main(String[] args) {
		...
		module.getFunctions().values().forEach(func -> {
			...
			var result = query.getAttribute(func, SimpleForAnalysisResult.class);
			for (var loop : result.loops())
				System.out.printf("%s, %s, %d, %b, %s, %s\n", ValueUtil.dumpIdentifier(loop.begin()),
						ValueUtil.dumpIdentifier(loop.end()), ValueUtil.getConstantInt(loop.step()), loop.inclusive(),
						loop.head().getName(),
						loop.body().getName());
		});
	}
