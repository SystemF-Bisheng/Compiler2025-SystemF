...
借鉴 MLIR, 在原中层 IR (下称 MIR, Middle level IR) 引入结构化控制流, 但改变尽可能少. 具体来说, 通过引入 `yield`, `if`, `for`, `while` 四条新指令以及相关的新结构**区域 (region)**, 实现结构化控制流.

## 规约说明

基本结构与 MIR 保持一致, 由 `Module`, `Function/GlobalDecl`, `BasicBlock`, `Operation` 依次构成. 其规约与 MIR 保持一致.

引入新结构**区域 (region)**. 称某些指令带有的, 包含一个或多个基本块的, 基于指令规约进行控制流转移管理的结构为一个区域. 区域中的基本块, 可以以指令 `yield` 结尾作为 terminal, 表示尝试离开区域. 区域中的块的 br 目标不能为区域外的块. 区域中的块可以用 `ret` 作为 terminal, 立即终止整个函数, 跳出所有区域. 区域变量引用符合作用域规则, 子区域可以引用父区域变量, 但父区域不能引用子区域变量, 区域不能引用兄弟区域变量, 否则为 ub. 区域内指令可以是有区域的指令, 即区域可以嵌套.
...
`if` 包含一个 `i32` 的 `cond` 作为条件, 一个 `then_region` 区域, 和一个可选的 `else_region` 区域, 没有返回.
...
`if` 所属区域中的 `yield` 只可是无参的, 表示离开区域, 继续执行 `if` 后一条指令.
...
```llvm
  cond_br %cond, then_bb1, else_bb1
then_bb1: [...]
...
then_bbn: [...]
else_bb1: [...]
...
当 else 区域不存在时, 则等效表示中 `head` 中 `cond_br` 的第二个目标为 `merge`, 且不存在 `else_bb*`.

区域中的 `yield` 在等效表示中等价于被替换为 `br merge`.
