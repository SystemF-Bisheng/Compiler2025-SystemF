...
def asm_riscv(rv: str, obj: str, exe: str) -> Optional[TestResult]:
  ...
  '''
  cmd = f'riscv64-linux-gnu-gcc -static {rv} lib/libsysy_riscv.a -o {exe}'
...
if __name__ == '__main__':
  ...
  for case in cases:
    print(f'compiling {case.name}...')
    if sys.platform.startswith('linux'):
      dir_name = os.path.join(*(['tmp'] + case.name.split('/')[0:-1]))
    elif sys.platform == 'win32':
      dir_name = os.path.join(*(['tmp'] + case.name.split('\\')[1:-1]))
    else:
      print(f'unsupported platform {sys.platform}')
      exit(-1)
    os.makedirs(dir_name, exist_ok=True)
    compile_command = f'{compiler.compile_cmd} -S {case.source_file} -o tmp/{case.name}.S'
